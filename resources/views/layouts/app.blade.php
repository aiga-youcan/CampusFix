...
<head>
    ...
    <title>@yield('title', 'CampusFix | EST Fquih Ben Salah')</title>
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col font-sans">
    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center space-x-6">
                <a href="{{ route('dashboard') }}" class="text-xs font-medium transition-colors {{ request()->routeIs('dashboard') ? 'text-slate-900 font-semibold' : 'text-slate-500 hover:text-slate-900' }}">
                    Tableau de bord
                </a>
                <a href="{{ route('signalements.index') }}" class="text-xs font-medium transition-colors {{ request()->routeIs('signalements.*') ? 'text-slate-900 font-semibold' : 'text-slate-500 hover:text-slate-900' }}">
                    Signalements
                </a>
                <a href="{{ route('signalements.create') }}" class="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-900 hover:bg-slate-800 text-white transition-colors">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    ...
                    <div x-show="open" class="absolute right-0 mt-2 w-80 bg-white rounded-xl border border-slate-200 py-2 z-50 text-xs" style="display: none;">
                        <div class="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                            <span class="font-semibold text-slate-800 text-xs">Pannes en attente</span>
                            @if($unreadCount > 0)
                        ...
                        <div class="max-h-72 overflow-y-auto divide-y divide-slate-100">
                            ...
                            <div class="p-3 hover:bg-slate-50 transition flex items-start justify-between gap-2">
                                <a href="{{ url('/signalements/' . $alert->id) }}" class="flex-1 block">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="font-medium text-slate-900 truncate max-w-[170px]">{{ $alert->title }}</span>
                                        <span class="text-[9px] font-semibold px-1.5 py-0.2 rounded border uppercase {{ $alert->severity == 'critique' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-slate-100 text-slate-700 border-slate-200' }}">
                                            {{ $alert->severity }}
                                        </span>
                                    </div>
                                    <div class="text-[11px] text-slate-500">
                                        📍 {{ $alert->location }} • <span class="text-slate-400">{{ $alert->created_at?->diffForHumans() ?? 'Récemment' }}</span>
                                    </div>
                                </a>
                                <form method="POST" action="{{ route('notifications.markRead', $alert->id) }}">
                                    @csrf
                        ...
                        <div class="px-4 pt-2 border-t border-slate-100 text-center">
                            <a href="{{ route('signalements.index') }}" class="text-slate-600 hover:text-slate-900 font-medium text-[11px]">
                ...
                <div class="text-right hidden sm:block border-l border-slate-200 pl-3">
                    <div class="text-xs font-medium text-slate-800">{{ auth()->user()->name }}</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 w-full">
        <div class="p-3 rounded-lg bg-rose-50 border border-rose-200 text-rose-800 text-xs mb-3">
